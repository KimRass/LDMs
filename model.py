...
# "We introduce a domain specific encoder that projects y to an intermediate representation (y) 2 RM d  , which is then mapped to the intermediate layers of the UNet via a cross-attention layer implementing"
...
d_tau = 100 # "$d_{\tau}$"
d = 100 # "$d$"
d_eps = 100 # "$d^{i}_{\epsilon}$"
cond_out = torch.randn((BATCH_SIZE, M, d_tau)) # "$\tau_{\theta}(y)$"

q_proj = nn.Linear(d_eps, d, bias=False) # "$W^{(i)}_{Q}$"
k_proj = nn.Linear(d_tau, d, bias=False) # "$W^{(i)}_{K}$"
v_proj = nn.Linear(d_tau, d, bias=False) # "$W^{(i)}_{V}$"

k = k_proj(cond_out) # "$K = W^{(i)}_{K} \cdot \tau_{\theta}(y)$"
v = v_proj(cond_out) # "$V = W^{(i)}_{V} \cdot \tau_{\theta}(y)$"
